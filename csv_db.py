...
def convert_datetime_columns(df, datetime_columns):
    ...
    for col in datetime_columns:
        if col in df.columns:
            df.loc[:, col] = pd.to_datetime(df[col], errors="coerce")  # Convert to datetime format
            
            # Only format columns that are datetime types
            if pd.api.types.is_datetime64_any_dtype(df[col]):
                df.loc[:, col] = df[col].dt.strftime("%Y-%m-%d %H:%M:%S")  # Format for MySQL
            else:
...
def setOrganizationalUnits(conn):
    ...
    filtered_content_objects_df['Recorded'] = 0
...
def setDb(db_config):
    ...
    print('Running OrganizationalUnits and Content Objects...')
    setOrganizationalUnits(conn)
    print('OrganizationalUnits and ContentObject tables updated successfully.')

    print('Running OrganizationalUnitAncestors...')
    setAncestors(conn)
    print('OrganizationalUnitAncestors table updated successfully.')
...
def write_to_table(conn, table, df, table_columns, batch_size=1000):
    ...
    if not data:
        print(f"Skipping '{table}' as there are no records to insert.")
        cursor.close()
        return
...
def get_sylabus(db_config):
    ...
    syllabus_query = """
        ...
        WHERE ou.Year = 2024 
        AND ou.Term = 'FW' 
        AND f.ProjectId IS NOT NULL;  -- Exclude rows where ProjectId is NULL
    """
    conn = get_db_connection(db_config)
    cursor = conn.cursor()
    print("Executing syllabus query...")
    try:
        ...
        print(f"Syllabus query results saved to: {syllabus_csv_file}")

        return df_syllabus

    except mysql.connector.Error as err:
        print(f"Error executing syllabus query: {err}")
...
def update_syllabus_recorded(db_config, df, batch_size=1000):
    ...
    data = [(row['Recorded'], row['OrgUnitId']) for _, row in df.iterrows()]
    ...
    try:
        ...
        print(f"Error updating ContentObjects: {err}")
        conn.rollback()
    finally:
        cursor.close()
        conn.close()
